...
import { irParaHome, irParaTripDetails } from '../Rotas/coordinator'
...
const application = () =>{
  ...
  console.log(id)
  console.log(body)
  const url =`https://us-central1-labenu-apis.cloudfunctions.net/labeX/Lucas-Quevedo-Silveira/trips/${id}/apply`
  axios.post(url, body)
  .then((response) =>{
...
.catch((err)=>{
  // setName(err.response)
  console.log(err.response)
})
  
}

const listaViagens =viagem && viagem.map((detalhes) =>{
  return <option key={detalhes.id} value={detalhes.id}>
...
console.log(listaViagens)
  ...
  <Container>
    ...
    <form>
      ...
      <option value={""} disabled>Seleciona sua viagem</option>
      {listaViagens}
